...
#2
variant_q_hat[nrow(variant_q_hat), 1:2]

#3
p_hat = variant_q_hat[nrow(variant_q_hat), ]
n = colSums(variants_vol)

z = 1.96
lower = p_hat - z * sqrt(p_hat * (1 - p_hat) / n)
upper = p_hat + z * sqrt(p_hat * (1 - p_hat) / n)

cbind(lower, upper)[1:2, ]

#5
X_A = num_successes[1]
X_B = num_successes[2]

n_A = sum(variants_vol[,1])
n_B = sum(variants_vol[,2])

p_pooled = (X_A + X_B) / (n_A + n_B)
SE = sqrt(p_pooled * (1 - p_pooled) * (1/n_A + 1/n_B))

p_hat_A = variant_q_hat[nrow(variant_q_hat), 1]
p_hat_B = variant_q_hat[nrow(variant_q_hat), 2]
...
#7
Z_BA = -2.77
p_value_one_sided = 1 - pnorm(Z_BA)
p_value_one_sided


#8
prop.test(x = c(2*X_A, 2*X_B), n = c(2*n_A, 2*n_B), correct = FALSE)
#8

# Double the actual successes and volumes from your run
X_A_new = 2 * X_A
X_B_new = 2 * X_B
n_A_new = 2 * n_A
n_B_new = 2 * n_B

prop.test(x = c(X_A_new, X_B_new), n = c(n_A_new, n_B_new), correct = FALSE)

#8 new and improved
# ------------------------------------------
# Step 1: Extract final estimated conversion rates
# ------------------------------------------
# variant_q_hat: rows = iterations, cols = ads
p_hat <- variant_q_hat[nrow(variant_q_hat), 1:2]  # Ad A = 1, Ad B = 2
p_hat_A <- p_hat[1]
p_hat_B <- p_hat[2]

# ------------------------------------------
# Step 2: Extract total impressions per ad
# ------------------------------------------
n <- colSums(variants_vol)  # sum over all iterations
n_A <- n[1]
n_B <- n[2]

# ------------------------------------------
# Step 3: Double sample sizes (longer experiment)
# ------------------------------------------
n_A_new <- 2 * n_A
n_B_new <- 2 * n_B

# Keep conversion rates the same -> double successes
X_A_new <- round(p_hat_A * n_A_new)
X_B_new <- round(p_hat_B * n_B_new)

# ------------------------------------------
# Step 4: Calculate pooled proportion
# ------------------------------------------
p_pooled <- (X_A_new + X_B_new) / (n_A_new + n_B_new)

# ------------------------------------------
# Step 5: Standard error for Z-test
# ------------------------------------------
SE <- sqrt(p_pooled * (1 - p_pooled) * (1/n_A_new + 1/n_B_new))

# ------------------------------------------
# Step 6: Z-statistics
# Two-sided test
Z_two_sided <- (p_hat_A - p_hat_B) / SE

# One-sided test (H1: B > A)
Z_one_sided <- (p_hat_B - p_hat_A) / SE

# ------------------------------------------
# Step 7: p-values
p_two_sided <- 2 * (1 - pnorm(abs(Z_two_sided)))
p_one_sided <- 1 - pnorm(Z_one_sided)

# ------------------------------------------
# Step 8: Print results
# ------------------------------------------
cat("Doubled experiment results:\n")
cat("Two-sided Z =", round(Z_two_sided, 2), ", p-value =", signif(p_two_sided, 4), "\n")
cat("One-sided (B > A) Z =", round(Z_one_sided, 2), ", p-value =", signif(p_one_sided, 4), "\n")
